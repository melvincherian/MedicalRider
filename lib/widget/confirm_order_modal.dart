...
//     final Map<String, bool> _isUploading = {};
//     final Map<String, bool> _imagesUploaded = {};
//     final Map<String, double?> _locationErrorDistance = {};
...
//             _pharmacyImages[pharmacyId]!.add(File(image.path));
//             _imagesUploaded[pharmacyId] = false;
//             _locationErrorDistance.remove(pharmacyId);
//           });
//         }
//       } catch (e) {
//         _showErrorSnackbar('Failed to open camera');
...
//               if (mounted) {
//                 setState(() {
//                   _locationErrorDistance[pharmacyId] = distance;
//                 });
//               }
//             } catch (_) {
...
//               _locationErrorDistance.containsKey(currentPharmacy.pharmacyId) &&
//               _locationErrorDistance[currentPharmacy.pharmacyId] != null;
...
//     Widget _buildImageUploadSection({required String pharmacyId}) {
//       final bool hasLocationError =
//           _locationErrorDistance.containsKey(pharmacyId) &&
//           _locationErrorDistance[pharmacyId] != null;
...
//                 _locationErrorDistance[pharmacyId]!,
//                 pharmacyId,
//               ),
//           ],
//         ),
...
//     Widget _buildLocationErrorBanner(double distanceInMeters, String pharmacyId) {
...
//             Text(
//               'You are $distanceText away from this pharmacy. '
//               'Please go to the pharmacy before uploading pickup photos.',
//               style: const TextStyle(
//                 fontSize: 12,
...
//                 onPressed: () {
//                   // Find the current pharmacy to get coordinates
//                   final provider = Provider.of<RiderOrderProvider>(
//                     context,
//                     listen: false,
//                   );
//                   final pharmacy = provider.acceptedPharmacies.firstWhere(
//                     (p) => p.pharmacyId == pharmacyId,
//                     orElse: () => provider.acceptedPharmacies.first,
//                   );
//                   _openGoogleMaps(pharmacy.latitude, pharmacy.longitude);
//                 },
//                 style: ElevatedButton.styleFrom(
...
class _ConfirmOrderModalState extends State<ConfirmOrderModal> {
  ...
  void initState() {
    super.initState();
    // FIX: Use addPostFrameCallback so the widget tree is fully built
    // before we access the Provider. In release/Play Store builds the
    // navigation timing is tighter than debug, causing the old
    // `_loadOrder()` call in initState to run before the Provider is
    // ready, leaving the screen stuck on the loading spinner.
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        _loadOrder();
      }
  ...
  Future<void> _loadOrder() async {
    ...
    // FIX: Reset stale state from a previous session/order so isLoading
    // and currentOrder don't carry over and block the UI.
    provider.reset();
            ...
            body: Center(
              child: Column(
                ...
                children: [
                  const CircularProgressIndicator(),
                  const SizedBox(height: 16),
                  // FIX: Always show the Refresh button so the user is
                  // never permanently stuck on the spinner in release builds.
                  const Text(
                    'Loading order...',
